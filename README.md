...
- **매수** (미보유 시): `green_count ≥ 5` (실질금리+달러인덱스 × 5/30/60일, 총 6개 셀 중 우호적인 셀 수) **또는** 금/은비율 ≥ 90
  — 이 두 조건은 매수 지연일수만큼 지난 뒤 체결. **또는** (화면의 체크박스로 켠 경우) 금(GC=F)
  종가가 분석 기간 내 신고가를 경신 — 이 조건은 지연 없이 항상 당일 종가에 즉시 체결되며,
  아직 대기 중인 지연 매수 주문이 있어도 우선 체결됨
- **매도** (보유 시): `green_count = 0` **또는** 금/은비율 ≤ 40
- green_count/금은비율 신호의 체결가는 신호 발생일 기준, 화면에서 설정한 지연일수만큼 지난 뒤
  첫 거래일 종가(기본값 0일 = 신호 당일 즉시 체결). 예: 매수 지연일수를 30으로 설정하면
  "신호 5개 발생 후 약 1개월 뒤 매수"를 시뮬레이션
