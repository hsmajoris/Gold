...
- 배경 세로 음영(옅은 빨강) = 그 지표 기준 매수신호 활성 구간. 실질금리·달러인덱스·WTI·VIX는
  5/30/60일 이평선 3개를 모두 동시에 만족하는 날, 금/은비율은 값이 매수 임계값(기본 90) 이상인
  날에 표시됩니다. 이 판정은 `gold_dashboard/signals.py`의 단일 함수(`gold_friendly_vs_ma`)를
  통해 이뤄지며, 메인 표의 셀 하이라이트(`build_table.py`)·백테스트의 green_count 계산
  (`backtest.py`)도 전부 같은 함수를 호출합니다 — 이평선 하회/상회 판정 코드가 세 곳에 따로
  구현되어 있지 않습니다.
  - 다만 green_count는 실질금리+달러인덱스 두 지표의 판정을 **합산**한 값(0~6)이라, 그래프 하나의
    음영(그 지표 혼자 3개를 모두 만족하는지)만으로는 실제 green_count≥5 매수 신호와 정확히
    일치하지 않을 수 있습니다 — 예를 들어 실질금리 3/3 + 달러인덱스 2/3 = 5로 매수가 발생하면
    실질금리 그래프에는 음영이 뜨지만 달러인덱스 그래프에는 뜨지 않습니다(합계가 5 이상이면
    수학적으로 둘 중 최소 하나는 반드시 3/3을 만족하므로, 매수 시점에 최소 한 그래프에는 항상
    음영이 있습니다). 금/은비율 단독 트리거로 발생한 매수는 애초에 실질금리·달러인덱스 조건과
