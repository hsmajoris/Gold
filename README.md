...
메인 표 바로 아래, 지표별로 "그래프 보기" 버튼이 있습니다. 누른 지표만 그 시점에 최근 7년치
데이터를 받아와 그립니다 — 페이지를 열었을 때나 다른 지표 버튼을 눌렀을 때는 계산이 실행되지
않습니다(`st.session_state`로 지표별 펼침 여부를 따로 관리하고, 그 플래그가 켜진 지표에
대해서만 데이터를 요청하는 조건부 실행 패턴). 데이터는 하루(TTL 86400초) 캐시됩니다.
...
- 7년치 fetch 로직은 `gold_dashboard/timeseries.py`에 공통 모듈로 분리되어 있으며, 백테스트
