...
- **매수** (미보유 시): `green_count ≥ 5` (실질금리+달러인덱스 × 5/30/60일, 총 6개 셀 중 우호적인 셀 수)
  — 매수 지연일수만큼 지난 뒤 체결. **또는** 금/은비율 ≥ 화면에서 설정한 매수 임계값(기본 90)
  — 지연 없이 항상 당일 종가에 즉시 체결. **또는** (화면의 체크박스로 켠 경우) 금(GC=F) 종가가
  분석 기간 내 신고가를 경신 — 이것도 지연 없이 즉시 체결. 금/은비율·신고가 갱신은 아직 대기
  중인 green_count 지연 매수 주문이 있어도 우선 체결됨
- **매도** (보유 시): `green_count = 0` — 매도 지연일수만큼 지난 뒤 체결. **또는** 금/은비율 ≤
  화면에서 설정한 매도 임계값(기본 40) — 지연 없이 항상 당일 종가에 즉시 체결(마찬가지로 대기 중인
  green_count 지연 매도 주문을 우선함)
- 매수/매도 금/은비율 임계값은 화면에서 자유롭게 조정 가능 (예: 매도 임계값을 40 대신 50이나 60으로
  올려 더 자주 매도하는 시나리오도 테스트 가능)
- green_count 신호의 체결가는 신호 발생일 기준, 화면에서 설정한 지연일수만큼 지난 뒤 첫 거래일
  종가(기본값 0일 = 신호 당일 즉시 체결). 예: 매수 지연일수를 30으로 설정하면
  "신호 5개 발생 후 약 1개월 뒤 매수"를 시뮬레이션. 금/은비율·신고가 갱신 신호에는 지연이 적용되지 않음
